...
``` r
  ggplot(      data = data_chart,                       aes(x = date, colour = country ) ) + 
    ...
    geom_text( data = data_chart %>% filter(max_date == 1), aes(label = label, colour = country, x = date, y = value_f), 
               vjust = 0, nudge_x = 0.35, size = 3) +
    labs(y=paste0(ifelse(TAKE_LOG, "Log of",""),"Number of cases in 3 days"), x = "Date") +
    labs(title = "5.1 Number of cases forecast", subtitle = "countries > 1000 + Poland") +
