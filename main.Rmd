...
#            TAKE_LOG = TRUE,
#            labs_y = "Log(Number of cases)",
#            title = "1.1 Log(Number of cases), Day 0 = 1 case",
#            )
```
...
#            TAKE_LOG = TRUE,
#            labs_y = "Log(Number of cases)",
#            title = "1.2 Log(Number of cases), Day 0 = 30 cases"
#            )
...
plot_chart(data = build_data(START_CASES_NO = 30),
          TAKE_LOG = FALSE,
          labs_y = "Number of cases per 1M inhabitants",
          title = "3.2 Number of cases per 1M inhabitans, Day 0 = 30 cases"
          )
```
...
conf_filtered <- build_data(START_CASES_NO = 1, cumulative = TRUE) %>%
  filter(value >0 & tests > 0) %>%
  mutate(tests = log(tests)) %>%
  mutate(cases_per_test = log(cases_per_test)) %>%
...
ggplot(conf_filtered, aes(tests, cases_per_test, colour = country)) + 
  ...
  labs(title = "5.1 Number of tests per 1M inhabitants vs cases per test, cumulative", subtitle = paste0("countries > ",MIN_CASES_CUM," nominal cases + Poland. Point label = date")) +
  labs(caption = "source: github.com/CSSEGISandData/COVID-19, finddx.org/covid-19/test-tracker") +
  theme(legend.position = "none")
```

```{r, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
conf_filtered <- build_data(START_CASES_NO = 1) %>%
  filter(value > 0 & tests > 0) %>%
  mutate(tests = log(tests)) %>%
  mutate(cases_per_test = log(cases_per_test)) %>%
  filter(date > MIN_DATE_TESTS)
...
plot_chart(data = build_data(START_DEATHS_NO = 10, dead = TRUE, cumulative = FALSE, MA = 3),
          TAKE_LOG = TRUE,
          dead = TRUE,
          labs_y = "Logged number of deaths per 1M inhabitants, 3-day moving average",
          title = "8.1 Logged number of deaths per 1M inhabitans, Day 0 = 10 deaths, 3-day moving average"
          )
```
...
plot_chart(data = build_data(START_DEATHS_NO = 10, dead = TRUE, cumulative = FALSE, MA = 3),
          ...
          dead = TRUE,
          labs_y = "Logged number of deaths per 1M inhabitants, 3-day moving average",
          title = "8.2 Number of deaths per 1M inhabitans, Day 0 = 10 deaths, 3-day moving average"
          )
```