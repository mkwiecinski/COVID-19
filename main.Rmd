...
```{r, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
countries_include <- c("Poland")
conf_filtered <- build_data(START_CASES_NO = 1, MIN_CASES = 2000, MAX_CASES = Inf, CHART_MAX = 30000, countries_include)
...
ggplot(conf_filtered, aes(days, value, colour = country)) + 
  ...
  labs(title = "2.1 Number of cases, Day 0 = 1 case", subtitle = "countries > 2000 + Poland, crop at 30000") +
  labs(caption = "data source: github.com/CSSEGISandData/COVID-19")
```
...
```{r, echo=FALSE, error=FALSE, warning=FALSE, message=FALSE}
countries_include <- c("Poland")
conf_filtered <- build_data(START_CASES_NO = 30, MIN_CASES = 2000, MAX_CASES = Inf, CHART_MAX = 30000, countries_include)
...
ggplot(conf_filtered, aes(days, value, colour = country)) + 
  ...
  geom_text(data = conf_filtered %>% filter(maxv == 1), aes(label = country, colour = country, x = days, y = value), hjust = -.1) +
  labs(y="Number of cases", x = "Days from patient 0") +
  labs(title = "2.2 Number of cases, Day 0 = 30 cases", subtitle = "countries > 2000 + Poland, crop at 30000") +
  labs(caption = "data source: github.com/CSSEGISandData/COVID-19")
```
...
ggplot(conf_filtered, aes(tests_per_1M, cases_per_test, colour = country)) + 
  ...
  labs(title = "4.1 Number of tests per 1M inhabitants vs cases per test", subtitle = "countries > 2000 nominal cases + Poland. Point label = date") +
